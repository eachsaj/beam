...
public final class SparkCompat {
  ...
  /**
   * A class that wraps an {@link Iterator} and exposes it as an {@link Iterable} or
   * wraps an {@link Iterable} and exposes it as an {@link Iterator}. In the second case it
   * creates an Iterator by calling the iterable.Iterator method and then respects its semantics.
   *
   * <p>Notice that both uses cannot be combined, so it is up to the user to respect the logic
   * since we cannot ensure this.
   *
  ...
  /**
   * A SparkCompat.IterableIterator compatible version of
   * {@link org.apache.spark.api.java.function.FlatMapFunction}.
   */
  public interface FlatMapFunction<K, V>
      extends org.apache.spark.api.java.function.FlatMapFunction<K, V> {

    @Override SparkCompat.IterableIterator<V> call(K k) throws Exception;
  }

  /**
   * A SparkCompat.IterableIterator compatible version of
   * {@link org.apache.spark.api.java.function.PairFlatMapFunction}.
   */
  public interface PairFlatMapFunction<T, K, V>
      extends org.apache.spark.api.java.function.PairFlatMapFunction<T, K, V> {

    @Override SparkCompat.IterableIterator<Tuple2<K, V>> call(T t) throws Exception;
  }
}
