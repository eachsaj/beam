...
public class GlobalWatermarkHolder {
  ...
  public static void advance(final String batchId) {
    synchronized (GlobalWatermarkHolder.class) {
      final SparkCompat.BlockManager scbm = SparkCompat.newBlockManager();
      final Map<Integer, SparkWatermarks> newWatermarks = computeNewWatermarks(scbm);

      if (!newWatermarks.isEmpty()) {
        writeRemoteWatermarkBlock(newWatermarks, scbm);
   ...
   */
  private static Map<Integer, SparkWatermarks> computeNewWatermarks(BlockManager scbm) {
    ...
    for (final Map.Entry<Integer, Queue<SparkWatermarks>> watermarkInfo: sourceTimes.entrySet()) {
      ...
      final Map<Integer, SparkWatermarks> currentWatermarks = initWatermarks(
          scbm);
  ...
  private static void writeRemoteWatermarkBlock(final Map<Integer, SparkWatermarks> newWatermarks,
      final SparkCompat.BlockManager scbm) {
    scbm.removeBlock(SparkCompat.BlockManager.WATERMARKS_BLOCK_ID, true);
  ...
  private static Map<Integer, SparkWatermarks> initWatermarks(final SparkCompat.BlockManager scbm) {

    final Map<Integer, SparkWatermarks> watermarks = fetchSparkWatermarks(scbm);

    if (watermarks == null) {
      final HashMap<Integer, SparkWatermarks> empty = Maps.newHashMap();
      scbm.putSingle(empty);
      return empty;
    } else {
      return watermarks;
    }
  ...
  private static Map<Integer, SparkWatermarks> fetchSparkWatermarks(final BlockManager
      scbm) {
    final Option<BlockResult> blockResultOption = scbm.getRemote();
  ...
  public static synchronized void clear() {
    ...
    SparkCompat.newBlockManager().removeBlock(SparkCompat.BlockManager.WATERMARKS_BLOCK_ID, true);
  }
